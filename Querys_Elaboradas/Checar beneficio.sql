WITH parametros_periodo AS (
    SELECT DATE '2025-10-01' AS dt_inicio,
           DATE '2025-11-01' AS dt_fim
    FROM dual
),

-- CTE para lista de fornecimentos
forn AS (


   select distinct id_fornecimento from ora_dw.cad_fornecimento where id_fornecimento in (
'48163406001',
'48290181003',
'48289922002',
'48289337001',
...
faturas_filtradas AS (
    ...
    WHERE FATURA.CD_SECCIONAL IN ('SOR', 'SOF')
      AND FATURA.ID_FATURA NOT IN (
          SELECT ID_FATURA_DESTINO FROM ORA_DW.MOV_REFATURAMENTO
      )
...
SELECT 
    ...
    BENE.TP_BENEFICIO,
    DESC_BEN.DS_TP_BENEFICIO,
    BENE.DT_INI_BENEFICIO,
    BENE.DT_FIM_BENEFICIO,
        
    
    CASE
        WHEN BENE.DT_FIM_BENEFICIO > TRUNC(SYSDATE) THEN 'BENEFICIO ATIVO'  --TO_DATE('31/12/9999', 'DD/MM/YYYY')
        WHEN BENE.DT_FIM_BENEFICIO < TRUNC(SYSDATE) THEN 'BENEFICIO ENCERRADO'
...
FROM
    ...
    LEFT JOIN ORA_DW.REL_FORNECIMENTO_BENEFICIO BENE ON FORN.ID_FORNECIMENTO = BENE.ID_FORNECIMENTO
    LEFT JOIN ORA_DW.TBA_TP_BENEFICIO DESC_BEN ON BENE.TP_BENEFICIO = DESC_BEN.TP_BENEFICIO

WHERE FATURA.ID_FORNECIMENTO IN (SELECT id_fornecimento FROM forn); --AND BENE.TP_BENEFICIO = '13';
